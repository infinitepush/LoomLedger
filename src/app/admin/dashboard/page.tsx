...
import { 
  ...
  ArrowRight, Search, Check, X, RefreshCw 
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] bg-background flex-grow">
      ...
      <main className="flex-1 p-6 sm:p-10 max-w-5xl space-y-8">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-serif font-semibold text-foreground">Registry Moderation Queue</h1>
          <p className="text-xs text-muted-foreground">Approve weaver applications and deploy digital identity contracts on Polygon Amoy ledger.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Total Saree Catalog', value: products.length, icon: Package, color: 'text-primary bg-primary-light' },
        ...
        <div className="space-y-4">
          ...
          {filteredPending.length === 0 ? (
            <div className="bg-white border border-border p-16 rounded-xl text-center space-y-3">
                  ...
                  <div className="space-y-4 flex-grow">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">Origin Cluster: {artisan.region}</p>
                      </div>
