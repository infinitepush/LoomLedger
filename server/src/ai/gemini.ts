...
export class GeminiService {
  ...
  async askAssistantFallback(question: string): Promise<string> {
    if (this.ai) {
      try {
...
Provide a concise, helpful response (max 150 words).`;
        ...
        logger.error('Gemini assistant fallback failed:', err);
      }
    }
  ...
  async generateProductSpecs(productName: string, category: string): Promise<{
    description: string;
    fabric: string;
    craftTime: string;
    ...
    if (this.ai) {
      try {
        ...
        logger.error('Gemini call failed, using simulation fallback:', err);
      }
    }
  ...
  async generateArtisanStory(name: string, craft: string, region: string): Promise<string> {
    if (this.ai) {
      try {
        ...
        logger.error('Gemini story call failed:', err);
      }
    }
