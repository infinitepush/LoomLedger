import { PrismaClient, UserRole, ArtisanStatus, OrderStatus, PaymentStatus } from '@prisma/client';
  ...
  for (const art of artisansData) {
    ...
    const artisan = await prisma.artisan.create({
      data: {
        ...
        walletAddress: '0x' + (uuidv4().replace(/-/g, '') + uuidv4().replace(/-/g, '')).substring(0, 40),
        verificationHash: '0x' + uuidv4().replace(/-/g, ''),
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        blockchainTxHash: '0x' + uuidv4().replace(/-/g, '') + uuidv4().replace(/-/g, '').substring(0, 24),
    ...
    await prisma.payment.create({
      data: {
        orderId: order.id,
        razorpayOrderId: `rzp_order_${uuidv4().substring(0, 10)}`,
        razorpayPaymentId: `rzp_pay_${uuidv4().substring(0, 10)}`,
